...
public class UserController {
    ...
    public ResponseEntity<User[]> getUsers() {
        try {
            return new ResponseEntity<>(  this.jsonPlaceholderService.allUsers(),HttpStatus.OK);
    ...
    public ResponseEntity<Album[]> getAlbumsFromIdUser(@PathVariable String id) {
        try {
            return new ResponseEntity<>(this.jsonPlaceholderService.allAlbumsOfUser(Long.valueOf(id)),HttpStatus.OK);
        } catch (Exception exception) {
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
